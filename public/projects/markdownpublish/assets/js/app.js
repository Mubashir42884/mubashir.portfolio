...
`;
  ...
  function loadState() {
    ...
    try {
      ...
      if (prefs.editorFont) {
        document.getElementById('editorFontSelect').value = prefs.editorFont;
      }

      if (prefs.previewFont) {
        document.getElementById('previewFontSelect').value = prefs.previewFont;
      }
      ...
      if (prefs.editorWidth) {
        document
          .getElementById('workspace')
          .style.setProperty('--editor-width', prefs.editorWidth);
      }
    } catch (_) {}
  ...
  function render() {
    const settings =
      window.MDPMarkdown.parseDocumentSettings(
        editor.value
      );

    lastRawHtml =
      window.MDPMarkdown.renderMarkdown(
        editor.value
      );
  ...
  function updateStats() {
    const body =
      window.MDPMarkdown.stripDocumentSettings(
        editor.value
      );
  ...
  function saveMarkdown() {
    ...
    window.MDPExport.downloadBlob(
      editor.value,
      'text/markdown;charset=utf-8',
      window.MDPExport.safeFilename(
        titleInput.value,
        'md'
      )
    );
  ...
  function saveHtml() {
    ...
    const html =
      window.MDPExport.buildStandaloneHtml({
        title:
          titleInput.value,
  ...
  function updateThemeButton() {
    ...
    if (faviconLink) {
      faviconLink.href =
        isDark
          ? 'assets/images/markdownpublish-icon-dark.svg'
  ...
  function updateHighlightTheme() {
    ...
    themeLink.href =
      isDark
        ...
        : 'vendor/highlight/atom-one-light.min.css';
  }
  ...
  function setPreviewMode(
    ...
    previewScroll.classList.toggle(
      'pdf-mode',
      pdf
    );
  ...
  function newDocument() {
    ...
    if (
      ...
      !window.confirm(
        'Start a new document? Your current draft is already autosaved locally.'
      )
    ) {
      return;
    }
  ...
  function saveHeaderFooterSettings() {
    ...
    editor.value =
      window.MDPMarkdown.applyDocumentSettings(
        editor.value,
        settings
      );
    ...
    showToast(
      'Print header/footer settings applied.'
    );
  }
  ...
  // =========================================================
  //
  // Native editing shortcuts must remain under browser control.
  //
  // IMPORTANT:
  // We DO NOT call preventDefault() for:
  //
  ...
  //
  // We only stop propagation so another app-level handler
  // cannot interfere with them.
  //

  const nativeEditorShortcutKeys =
    new Set([
      'a',
      'c',
      'v',
  ...
  editor.addEventListener(
    ...
    (e) => {
      ...
      // Let the browser perform its standard
      // textarea editing operation.
      if (
        primary &&
        nativeEditorShortcutKeys.has(
          key
        )
      ) {
        e.stopPropagation();
  ...
  // App-wide Save Markdown shortcut.
  //
  // Native textarea editing commands do not reach this
  // listener because the editor handler above stops
  // their propagation.
